# coyied from https://github.com/pytorch/pytorch/issues/3867#issuecomment-974159134
import collections
...
class Conv2dSame(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, **kwargs):
        ...
        for d, k, i in zip(dilation_, kernel_size_, range(len(kernel_size_) - 1, -1, -1)):
            total_padding = d * (k - 1) - stride + 1 # (S/2-1)*W-S+1 + D(K-1) when stride = 2
...
class ConvTranspose2dSame(nn.Module):
    '''
    For stride = 2, padding = 0, always give one more px than expected, so mannually remove.
    '''

    def __init__(self, in_channels, out_channels, kernel_size, stride=2, dilation=1, **kwargs):
        ...
        # # https://github.com/pytorch/pytorch/blob/v1.10.0/torch/nn/modules/conv.py#L116
        # for d, k, i in zip(dilation_, kernel_size_, range(len(kernel_size_) - 1, -1, -1)):
        #     total_padding = d * (k - 1) - 1 # (S-2)W-S+1+(K-1)D when S = 2
        #     left_pad = total_padding // 2
        #     self._reversed_padding_repeated_twice[2 * i] = left_pad
