...
def train_and_test(model, optimizer, data_loader_train, data_loader_test, device, param, saveckp):
    ...
    for i, ((images_train, targets_train), (images_test, targets_test)) in enumerate(zip(data_loader_train, data_loader_test)):
        ...
        if (saveckp is not None) and hasattr(param, 'ckpt_path') and param.ckpt_path is not None:
          ...
          if check==0 and i > (saveckp-2):
            checkpoint = {"model": model.state_dict(), "epochs": i, "losses": {'train_loss': trainloss, 'test_loss': testloss}}
        ...
        if i % param.print_freq == 0:
            ...
            print("Epoch{}\t".format(i), "Test Loss {:.3f}".format(trainloss.item()), 'Cost: {}\t s.'.format(time.time()-record) )
